...
delay:	li	$a1, 0x01
	...
	addi	$s0, $0, 0		# i = 0
	addi	$t0, $0, 20		#$t0 = 1000, tal vi jämför med (i < 4711)
...
time2string:
	...
	sb	$v0, 0($s1)		# Lägger byten på rätt ställe i $v0
	...
	jal	hexasc			# skickar till hexasc och länkar tillbaka hit med $ra
	nop				# Ett mellanrum ifall att
	sb	$v0, 1($s1)		# Lägger byten på rätt ställe i $v0
	...
	jal	hexasc			# skickar till hexasc och länkar tillbaka hit med $ra
	nop				# Ett mellanrum ifall att
	sb	$v0, 3($s1)		# Lägger byten på rätt ställe i $v0
	...
	jal	hexasc			# skickar till hexasc och länkar tillbaka hit med $ra
	nop				# Ett mellanrum ifall att
	sb	$v0, 4($s1)		# Lägger byten på rätt ställe i $v0
	
	#Null/X
	andi	$t0, $s0, 0xFF		# maskar sekunderna
	beq	$t0, $zero, X		# branchar om sekunderna är noll
	nop
	
	li	$v0, 0x00
	sb	$v0, 5($s1)
	j	Skip			# för att skippa X, annars blir alla X
	nop
	
X:	li	$v0, 0x58		# ASCII för stora X
	sb	$v0, 5($s1)		# lägger den på rätt plats
	li	$v0, 0x00	
	sb	$v0, 6($s1)		# lägger null efteråt för att inte skriva "text more text lots of text" eller nåt
